...
- **17 objects in Playground.unity need the `Cargo` component re-attached — in progress, stuck on a real quirk.** Full story: `WulframSceneCheck` found 18 missing script references in `Playground.unity` (0 in `Launcher.unity`). 16 were on objects named `Cargo`/`Cargo (N)`, all broken at the same component slot; the 17th pattern-matched too (plain `Cargo`); the 18th (`RedBase/RML/Turret_SAM`) matched no class anywhere in the codebase.
  ...
  - **Phase 2 (re-attaching `Cargo` to the 17 objects): NOT done, uncommitted, reverted back to the Phase 1 checkpoint each time.** `Com.Wulfram3.Cargo` (`Assets/BlueFiles/cargo.cs`) is confirmed the correct class — `GameManager.cs` calls `GetComponent<Cargo>()` on these objects directly, and it's a real, currently-compiling class. The problem is persistence, not correctness: calling `go.AddComponent<Cargo>()` in a loop over all 17 objects, then one `EditorSceneManager.SaveScene` at the end, only reliably saves a **subset** — 1 of 17 with no explicit dirty-marking, 8 of 17 after adding `EditorUtility.SetDirty`/`EditorSceneManager.MarkSceneDirty` per object. **Which 8 succeed doesn't correlate with name, numeric suffix, or processing order** — tried and ruled out. This looks like a genuine non-deterministic quirk in batch-mode/`-executeMethod` Unity sessions doing many structural mutations without any Editor update-loop pumping between them, not a logic bug in the fix script itself.
  - **Two known-viable next options, not yet tried:** (a) one separate Unity CLI process per object (17 launches instead of 1) — slower, but each process does the minimum possible work before saving, which should sidestep whatever this in-process batching issue is; (b) do this specific step manually in the actual Unity GUI (opening the project normally, not batch mode) — for exactly 17 objects this might genuinely be faster than continuing to fight batch-mode automation, and sidesteps the issue entirely since there's no reason to believe the GUI has the same problem.
